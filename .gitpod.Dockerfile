...
RUN touch /DOCKERFILERUN \
	...
	&& apt-get -y update \
	&& apt-get -y -t testing install python3 strace gdb clang golang \
	&& apt-get -y -t testing install tmux netcat-openbsd socat procps net-tools curl wget lynx links \
